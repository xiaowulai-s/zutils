# 开发工具箱

一个功能丰富的在线工具集，专为电子工程师和开发者设计。

## 功能概览

### 硬件/PCB 工具
- **PCB 过孔电流计算器** - 基于 IPC-2152 标准
- **PCB 走线宽度计算器** - 基于 IPC-2152 标准
- **二进制文件查看器** - 十六进制和 ASCII 视图
- **电池续航计算器** - 功耗和寿命计算
- **LED 限流电阻计算器** - 支持串并联配置
- **电阻色环阻值计算器** - 4/5/6 环电阻
- **SMD 电阻丝印助手** - 三位、四位及 EIA-96 编码
- **RC/RL 滤波器计算器** - 低通与高通滤波器

### 嵌入式开发
- **ESP32 在线烧录** - Web Flash 工具
- **电路在线仿真** - 支持 Arduino、ESP32、STM32

### 串口/网络调试
- **Web 串口助手** - Web Serial API
- **HTTP 请求构造器** - GET/POST/PUT/DELETE

### 进制转换与位操作
- **ASCII/字符转换**
- **进制转换器** - 二/八/十/十六进制
- **单位转换器** - 长度/重量/温度/面积/体积/速度/数据/时间

### 开发工具
- **JSON 格式化** - 美化、压缩、校验
- **正则表达式测试** - 实时匹配
- **正则表达式可视化** - 图形化解析正则
- **时间戳转换** - Unix 时间戳
- **AI 聊天助手** - OpenAI 兼容 API
- **代码对比** - LCS 算法精确对比
- **密码生成器** - 安全随机密码
- **代码片段管理** - 保存和管理代码
- **Markdown 预览** - 实时渲染
- **SQL 格式化** - 语句美化
- **Cron 表达式生成器** - 可视化生成
- **Git 命令速查** - 常用命令速查表
- **HTML 格式化** - HTML 代码美化
- **YAML 格式化** - YAML 格式化校验
- **文本差异对比** - 字符级/单词级精确对比
- **随机数据生成器** - 批量生成测试数据
- **Lorem Ipsum 生成器** - 占位文本生成
- **代码运行沙箱** - JavaScript/TypeScript/Python 在线运行
- **JSON Schema 生成器** - 从 JSON 自动生成 Schema
- **代码转换器** - JSON/YAML/CSV/TypeScript 格式互转

### 网络调试
- **WebSocket 测试** - 实时 WebSocket 连接测试
- **HTTP 客户端** - HTTP 请求测试工具
- **API Mock 服务器** - 创建模拟 API 端点

### 设计工具
- **颜色选择器** - HEX/RGB/HSL
- **二维码生成器** - 自定义二维码
- **图片 Base64 转换** - 互转工具
- **图片压缩** - 本地无损压缩
- **颜色对比度检测** - WCAG 无障碍标准
- **图片处理工具** - 裁剪、旋转、调整亮度/对比度
- **PDF 工具套件** - 合并、拆分、压缩、转图片

### 加密安全
- **CRC 计算** - 32 种标准算法
- **Hash/MD5/SHA** - 多种哈希算法
- **Base64 编解码**
- **URL 编解码**
- **UUID 生成器** - v1/v4 版本
- **JWT 解码器** - 解析和验证
- **加密解密工具** - AES 加密解密

## 技术栈

- **框架**: Next.js 15 (App Router)
- **语言**: TypeScript
- **样式**: Tailwind CSS
- **图标**: Lucide React
- **部署**: GitHub Pages

## 本地开发

```bash
# 安装依赖
npm install

# 启动开发服务器
npm run dev

# 构建静态文件
npm run build
```

## 部署

项目配置了 GitHub Actions 自动部署，推送到 main 分支后自动构建并部署到 GitHub Pages。

### 手动部署

1. 在 GitHub 仓库设置中启用 Pages
2. 选择 "GitHub Actions" 作为构建源
3. 推送代码后自动部署

访问地址: https://xiaowulai-s.github.io/zutils/

## 功能特性

- 实时搜索过滤
- 暗色模式切换
- 工具收藏功能
- 使用历史记录
- 键盘快捷键支持
- PWA 支持（可安装到桌面）
- 离线访问支持
- 响应式设计
- 数据本地存储（隐私安全）

## 快捷键

| 快捷键 | 功能 |
|--------|------|
| `/` 或 `Ctrl+K` | 聚焦搜索框 |
| `?` | 显示快捷键帮助 |

## 许可证

MIT License
